...
#define BUFSIZE  2048        //一次发送包的最大大小
...
int index_ip, index_arp, index_route, index_interface;//表项索引
FILE *fp;	//读取文件的指针


int re_sock_fd;         //接受链路层的包
int se_sock_fd;         //转发包
int n_read;
char recv_buf[BUFSIZE];    //接收包缓冲数组
char send_buf[BUFSIZE];    //转发包缓冲数组

char tmp_mac_addr[18], tmp_ip_addr[14];//读文件时暂存

struct sockaddr_ll saddr_11;        //物理地址结构
struct ifreq ifr;                   //.网络(网卡)接口数据结构

void init_file();					//初始化后各个配置

void StoMac(char *src, char *dst);  //把文件中读入的 MAC地址 转化为 6字节MAC
void StoIP(char *src, char *dst);   //把文件中读入的 IP地址  转化为 4字节IP
void Print_Mac(char *p);
